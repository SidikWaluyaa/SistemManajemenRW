<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('transaksi.update', $transaction->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <!-- Kategori (Optional) -->
                        <div class="mb-4">
                            <x-input-label for="kategori_transaksi_id" :value="__('Kategori')" />
                            <select id="kategori_transaksi_id" name="kategori_transaksi_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('kategori_transaksi_id', $transaction->kategori_transaksi_id) == $category->id ? 'selected' : '' }} data-nominal="{{ $category->nominal_default }}">
                                        {{ $category->nama_kategori }} ({{ $category->jenis }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4" id="warga-container" style="{{ $transaction->tipe === 'Pemasukan' ? '' : 'display: none;' }}">
                            ...
                            <select id="warga_id" name="warga_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                ...
                                @foreach($wargas as $warga)
                                    <option value="{{ $warga->id }}" {{ old('warga_id', $transaction->warga_id) == $warga->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <x-text-input id="jumlah" class="block mt-1 w-full" type="number" name="jumlah" :value="old('jumlah', $transaction->jumlah)" required />
                                <x-input-error :messages="$errors->get('jumlah')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="tanggal_transaksi" :value="__('Tanggal Transaksi')" />
                                <x-text-input id="tanggal_transaksi" class="block mt-1 w-full" type="date" name="tanggal_transaksi" :value="old('tanggal_transaksi', $transaction->tanggal_transaksi instanceof \Carbon\Carbon ? $transaction->tanggal_transaksi->format('Y-m-d') : $transaction->tanggal_transaksi)" required />
                        ...
                        <div class="mb-4">
                            ...
                            @if($transaction->bukti_foto)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $transaction->bukti_foto) }}" alt="Bukti Foto" class="h-20 w-auto rounded border">
                                </div>
                            @endif
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // toggleWarga(); // Don't run on load if we want to respect server-side rendering state? Actually checking display:none above covers it. But let's keep event listener.
